...
def vector(list1, list2):
    if len(list1) != len(list2):
        if len(list1) <= len(list2):
            ...
            toevoeging_1 = verschil_1 * list1.append(0)
            return list1 + list2
        if len(list2) <= len(list1):
            verschil_2 = len(list1) - len(list2)
            toevoeging_2 = verschil_2 * list2.append(0)
            return list1 + list2
    elif len(list1) == len(list2):
        return list1 + list2
    elif len(list1) and len(list2) > 1:
        multiplier = len(list1) - 1 
        toevoeging_1_ = multiplier * list1.append([])
        toevoeging_2_ = multiplier * list2.append([])
        return list1 + list2
vector(list1,list2)
